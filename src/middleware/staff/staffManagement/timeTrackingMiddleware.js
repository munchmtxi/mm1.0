...
async function checkTimeTrackingPermission(req, res, next) {
  try {
    const { user } = req; // Assuming user is attached by auth middleware
    if (!user || !staffConstants.STAFF_PERMISSIONS.manager.includes('manage_schedule')) {
